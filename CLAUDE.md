...
2. **`origin` has no embedded token**, so `git push` prompts for a username. A
   ...
   **⚠️ ADDED 2026-08-17: that token can PUSH but CANNOT open a pull request.**
   ...
   the same thing as a 403. It is a fine-grained PAT without `Pull requests:
   write`. The branch lands on the remote, so it looks like a partial success
   and it is easy to conclude the push failed when it did not: verify with

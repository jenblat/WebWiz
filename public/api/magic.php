...
try {
    ...
    $system .= "\n\n------\nSTRATEGIC FRAMEWORK — READ THIS BEFORE WRITING ANY HTML:\n\n"
            ...
            . "LOGO TREATMENT (NON-NEGOTIABLE - a logo must NEVER render as a blank box):\n"
            . "  - You CANNOT tell whether a scraped logo file has a transparent background or a baked-in white/light one. ASSUME IT HAS A SOLID LIGHT BACKGROUND unless the file is an .svg.\n"
            . "  - THEREFORE: never place a raw scraped logo image directly on a dark, black, navy, or saturated-colour surface. A white-background logo on a dark header renders as an EMPTY WHITE RECTANGLE. This is the single most common way these sites look broken, so treat it as a hard failure.\n"
            . "  - DEFAULT (use this unless you have a strong reason not to): render the business name as a styled TEXT WORDMARK in the nav/header. It always renders correctly and usually looks more designed than a small raster logo.\n"
            . "  - If you DO use the image logo in the nav, the surface directly behind it MUST be white or near-white (#ffffff to #f7f7f7): either make the whole header light, or sit the logo inside an explicit white rounded container with padding. Same rule anywhere else the logo appears.\n"
            . "  - NEVER apply filter:brightness(0) invert(1) (or any similar invert/brightness filter) to a scraped logo. That trick ONLY works on a transparent PNG. On a logo with a baked-in white background it turns the ENTIRE rectangle solid white and the logo disappears completely. This exact bug has shipped to a real customer. Do not use it.\n"
            . "  - Never let logos float as ugly white rectangles on cream either. If in doubt, use the wordmark.\n"
            . "  - In the footer, prefer a TEXT wordmark of the business name in the brand font, NOT a re-use of the small logo image. If you do use an image, use the same logo URL as the nav and the same light-surface rule applies.\n\n"

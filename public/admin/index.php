...
<header class="topbar">
  ...
  <nav>
    ...
    <a href="/admin/nurture.php" class="<?= $current==='nurture'?'on':'' ?>">Nurture</a>
    <?php if ($is_admin): ?>
      <a href="/admin/?tab=users" class="<?= $current==='users'?'on':'' ?>">Users</a>
      <a href="/admin/?tab=templates" class="<?= $current==='templates'?'on':'' ?>">Templates</a>
...
// ---------- TEMPLATES (admin-only) — industry library used by /try gens ----------
if ($tab === 'templates') {
    if (!$is_admin) { http_response_code(403); shell_open('Forbidden', $me, 'templates', $is_admin); echo '<h1>403</h1><p>Admins only.</p>'; shell_close(); exit; }

    // Schema (idempotent)
    ww_db()->exec("CREATE TABLE IF NOT EXISTS industries (
    ...
    // Seed 30 defaults if table is empty (first visit)
    $industry_seeds_path = __DIR__ . '/_industry_seeds.php';
    $seed_now = function () use ($industry_seeds_path) {
        if (!defined('WW_ADMIN_LOAD')) define('WW_ADMIN_LOAD', 1);
        $seeds = require $industry_seeds_path;
        if (!is_array($seeds)) return 0;
        $st = ww_db()->prepare("INSERT OR REPLACE INTO industries (slug, name, voice, hero, sections, banned, ctas, images, keywords, sort_order, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))");
        $i = 10;
        foreach ($seeds as $s) {
            $st->execute([(string)($s['slug'] ?? ''), (string)($s['name'] ?? ''), (string)($s['voice'] ?? ''), (string)($s['hero'] ?? ''), (string)($s['sections'] ?? ''), (string)($s['banned'] ?? ''), (string)($s['ctas'] ?? ''), (string)($s['images'] ?? ''), (string)($s['keywords'] ?? ''), $i]);
            $i += 10;
        }
        return count($seeds);
    };
    $first_seed = false;
    if ((int)ww_db()->query("SELECT COUNT(*) FROM industries")->fetchColumn() === 0) {
        $first_seed = true;
        $seed_now();
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        try {
            if ($action === 'save') {
                $id = (int)($_POST['id'] ?? 0);
                $slug = strtolower(trim(preg_replace('~[^a-z0-9_]+~i', '_', (string)($_POST['slug'] ?? ''))));
                $name = trim((string)($_POST['name'] ?? ''));
                if ($slug === '' || $name === '') throw new Exception('Slug and name are required.');
                $fields = ['voice','hero','sections','banned','ctas','images','keywords'];
                $vals = [];
                foreach ($fields as $f) { $vals[$f] = trim((string)($_POST[$f] ?? '')); }
                $active = isset($_POST['active']) ? 1 : 0;
                if ($id > 0) {
                    ww_db()->prepare("UPDATE industries SET slug=?, name=?, voice=?, hero=?, sections=?, banned=?, ctas=?, images=?, keywords=?, active=?, updated_at=datetime('now') WHERE id=?")
                        ->execute([$slug, $name, $vals['voice'], $vals['hero'], $vals['sections'], $vals['banned'], $vals['ctas'], $vals['images'], $vals['keywords'], $active, $id]);
                    $msg = "Updated '$name'.";
                } else {
                    ww_db()->prepare("INSERT INTO industries (slug, name, voice, hero, sections, banned, ctas, images, keywords, active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")
                        ->execute([$slug, $name, $vals['voice'], $vals['hero'], $vals['sections'], $vals['banned'], $vals['ctas'], $vals['images'], $vals['keywords'], $active]);
                    $msg = "Added '$name'.";
                }
            } elseif ($action === 'delete') {
                $id = (int)($_POST['id'] ?? 0);
                if ($id > 0) { ww_db()->prepare("DELETE FROM industries WHERE id=?")->execute([$id]); $msg = 'Deleted.'; }
            } elseif ($action === 'reseed') {
                ww_db()->exec("DELETE FROM industries");
                $n = $seed_now();
                $msg = "Reseeded $n defaults.";
            }
        } catch (Throwable $e) { $err = $e->getMessage(); }
        header('Location: /admin/?tab=templates' . ($msg ? '&msg=' . urlencode($msg) : '') . ($err ? '&err=' . urlencode($err) : ''));
        exit;
    }
    if (isset($_GET['msg'])) $msg = (string)$_GET['msg'];
    if (isset($_GET['err'])) $err = (string)$_GET['err'];

    $rows = ww_db()->query("SELECT * FROM industries ORDER BY sort_order ASC, name ASC")->fetchAll(PDO::FETCH_ASSOC);
    $edit_id = isset($_GET['edit']) ? (int)$_GET['edit'] : 0;
    $is_new = isset($_GET['new']);

    shell_open('Templates', $me, 'templates', $is_admin);
    ?>
    <style>
      .ind-msg{background:#d6f5e8;color:#1a4d36;border:2px solid #1a4d36;border-radius:10px;padding:10px 14px;margin-bottom:14px;font-weight:700;}
      .ind-err{background:#fcdfdf;color:#9b1c1c;border:2px solid #9b1c1c;border-radius:10px;padding:10px 14px;margin-bottom:14px;font-weight:700;}
      .ind-toolbar{display:flex;gap:10px;align-items:center;justify-content:space-between;margin-bottom:18px;flex-wrap:wrap;}
      .ind-toolbar h2{margin:0;font-size:22px;}
      .ind-toolbar-actions{display:flex;gap:8px;}
      .ind-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:14px;}
      .ind-card{background:#fff;border:2px solid var(--navy);border-radius:12px;padding:14px 16px;display:flex;flex-direction:column;gap:8px;box-shadow:3px 3px 0 var(--navy);}
      .ind-card h3{margin:0;font-size:16px;font-family:var(--display);}
      .ind-card .ind-slug{font-family:ui-monospace,monospace;font-size:11px;opacity:0.6;}
      .ind-card .ind-voice{font-size:12px;line-height:1.4;color:#444;flex:1;}
      .ind-card .ind-row{display:flex;gap:6px;margin-top:6px;}
      .ind-card .ind-row a, .ind-card .ind-row button{font-size:11px;padding:5px 10px;border-radius:999px;border:1.5px solid var(--navy);background:#fff;color:var(--navy);cursor:pointer;text-decoration:none;font-family:var(--display);font-weight:700;}
      .ind-card .ind-row a.edit{background:var(--yellow);}
      .ind-card .ind-row .del{background:transparent;color:#9b1c1c;border-color:#9b1c1c;}
      .ind-card.inactive{opacity:0.5;}
      .ind-edit-form{background:#fff;border:2px solid var(--navy);border-radius:14px;padding:18px 20px;margin-bottom:20px;box-shadow:4px 4px 0 var(--navy);}
      .ind-edit-form h2{margin:0 0 14px 0;font-size:18px;}
      .ind-edit-form label{display:block;font-family:var(--display);font-weight:800;font-size:12px;letter-spacing:0.04em;text-transform:uppercase;margin-bottom:4px;margin-top:12px;}
      .ind-edit-form input[type=text], .ind-edit-form textarea{width:100%;border:2px solid var(--navy);border-radius:8px;padding:8px 10px;font-family:var(--body);font-size:13px;background:#fff;color:var(--navy);}
      .ind-edit-form textarea{min-height:60px;resize:vertical;}
      .ind-edit-form .ind-row-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px;}
      .ind-edit-form .ind-actions{display:flex;gap:10px;margin-top:18px;align-items:center;}
      .ind-edit-form .ind-actions .save{background:var(--navy);color:var(--cream);border:0;padding:10px 18px;border-radius:999px;font-family:var(--display);font-weight:900;cursor:pointer;font-size:13px;}
      .ind-edit-form .ind-actions .cancel{background:transparent;color:var(--navy);border:2px solid var(--navy);padding:8px 16px;border-radius:999px;font-family:var(--display);font-weight:700;text-decoration:none;font-size:13px;}
      .ind-edit-form .ind-active{display:flex;align-items:center;gap:8px;font-size:13px;font-weight:600;}
      .ind-hint{font-size:11px;opacity:0.6;margin-top:3px;font-family:var(--body);font-weight:400;letter-spacing:0;text-transform:none;}
    </style>
    <div style="padding:20px 32px 80px;max-width:1200px;margin:0 auto;">
      <?php if ($msg): ?><div class="ind-msg"><?= ww_h($msg) ?><?php if ($first_seed) echo ' (first-time seed)'; ?></div><?php endif; ?>
      <?php if ($err): ?><div class="ind-err"><?= ww_h($err) ?></div><?php endif; ?>

      <div class="ind-toolbar">
        <div>
          <h2>Industry Templates</h2>
          <p style="margin:4px 0 0 0;font-size:13px;opacity:0.7;">Each industry biases the AI prompt for tone, sections, CTAs, and image rules. We auto-detect industry on every /try gen and inject the matching template into Sonnet's system prompt. Keywords are used for the auto-detect match.</p>
        </div>
        <div class="ind-toolbar-actions">
          <a href="/admin/?tab=templates&new=1" class="btn">Add industry</a>
          <form method="POST" style="display:inline;" onsubmit="return confirm('Wipe all industries and restore the 30 seeded defaults? Your edits will be lost.');">
            <input type="hidden" name="t_action" value="reseed">
            <button type="submit" class="btn ghost">Reseed defaults</button>
          </form>
        </div>
      </div>
      ...
      <?php
      $editing = null;
      if ($edit_id) { foreach ($rows as $r) if ((int)$r['id'] === $edit_id) { $editing = $r; break; } }
      if ($is_new) $editing = ['id'=>0,'slug'=>'','name'=>'','voice'=>'','hero'=>'','sections'=>'','banned'=>'','ctas'=>'','images'=>'','keywords'=>'','active'=>1];
      ?>
      <?php if ($editing): ?>
      <form class="ind-edit-form" method="POST">
        <input type="hidden" name="t_action" value="save">
        <input type="hidden" name="id" value="<?= (int)$editing['id'] ?>">
        <h2><?= $editing['id'] ? 'Edit "' . ww_h($editing['name']) . '"' : 'Add new industry' ?></h2>
        <div class="ind-row-grid">
          <div>
            <label>Name</label>
            <input type="text" name="name" value="<?= ww_h($editing['name']) ?>" required>
            <div class="ind-hint">Display name, e.g. "Restaurant (sit-down)"</div>
          </div>
          <div>
            <label>Slug</label>
            <input type="text" name="slug" value="<?= ww_h($editing['slug']) ?>" required pattern="[a-z0-9_]+">
            <div class="ind-hint">Lowercase, underscores. Stable ID — don't change once jobs reference it.</div>
          </div>
        </div>
        <label>Keywords (for auto-detect)</label>
        <input type="text" name="keywords" value="<?= ww_h($editing['keywords']) ?>">
        <div class="ind-hint">Comma-separated. Used to match the scraped page → this industry.</div>

        <label>Voice / tone</label>
        <textarea name="voice"><?= ww_h($editing['voice']) ?></textarea>
        <div class="ind-hint">How the copy should read.</div>

        <label>Hero treatment</label>
        <textarea name="hero"><?= ww_h($editing['hero']) ?></textarea>
        <div class="ind-hint">What the hero section should look like.</div>

        <label>Sections (allowed / preferred)</label>
        <textarea name="sections"><?= ww_h($editing['sections']) ?></textarea>

        <label>Banned sections / elements</label>
        <textarea name="banned"><?= ww_h($editing['banned']) ?></textarea>

        <label>CTAs (allowed)</label>
        <textarea name="ctas"><?= ww_h($editing['ctas']) ?></textarea>

        <label>Image rules</label>
        <textarea name="images"><?= ww_h($editing['images']) ?></textarea>

        <div class="ind-actions">
          <label class="ind-active"><input type="checkbox" name="active" <?= (int)$editing['active'] === 1 ? 'checked' : '' ?>> Active (eligible for auto-detect)</label>
          <button type="submit" class="save">Save</button>
          <a href="/admin/?tab=templates" class="cancel">Cancel</a>
          <?php if ($editing['id']): ?>
            <form method="POST" style="display:inline;margin-left:auto;" onsubmit="return confirm('Delete this industry?');">
              <input type="hidden" name="t_action" value="delete">
              <input type="hidden" name="id" value="<?= (int)$editing['id'] ?>">
              <button type="submit" style="background:transparent;border:2px solid #9b1c1c;color:#9b1c1c;padding:8px 16px;border-radius:999px;font-family:var(--display);font-weight:700;cursor:pointer;font-size:13px;">Delete</button>
            </form>
          <?php endif; ?>
        </div>
      </form>
      ...
      <div class="ind-grid">
        ...
        <div class="ind-card <?= ((int)$r['active']) ? '' : 'inactive' ?>">
          <h3><?= ww_h($r['name']) ?></h3>
          <div class="ind-slug"><?= ww_h($r['slug']) ?><?= (int)$r['active'] ? '' : ' · inactive' ?></div>
          <div class="ind-voice"><?= ww_h(mb_substr((string)$r['voice'], 0, 120)) ?><?= mb_strlen((string)$r['voice']) > 120 ? '…' : '' ?></div>
          <div class="ind-row">
            <a class="edit" href="/admin/?tab=templates&edit=<?= (int)$r['id'] ?>">Edit</a>
          </div>
        </div>
        <?php endforeach; ?>
      </div>
...
if ($tab === 'prospects') {
    ...
    if (!$batches) {
        ...
        foreach ($batches as $b) {
            ...
            $rest = [];
            if ($gen)  $rest[] = "$gen generating";
            if ($q)    $rest[] = "$q queued";
            if ($fail) $rest[] = "$fail failed";
            echo '<tr>';

...
<?php
// Build a Purchase event_id deterministically from the Stripe session_id so
// the client-side Pixel call and the server-side CAPI call from webhook.php
// share an event_id and Meta dedups them.
$_meta_eid = '';
if ($session_id) {
    $_meta_eid = 'ww_' . substr(hash('sha256', $session_id), 0, 24);
}
$_meta_amount = $amount !== null ? (float)$amount : 0.0;
$_meta_email  = (string)($email ?: '');
$_meta_plan   = (string)($plan ?: '');
?>
<script>
(function(){
  if(!window.wwMetaTrack) return;
  var eid = <?= json_encode($_meta_eid) ?>;
  var value = <?= json_encode($_meta_amount) ?>;
  var email = <?= json_encode($_meta_email) ?>;
  var plan = <?= json_encode($_meta_plan) ?>;
  if(!eid || !value) return;
  // Skip pixel-helper-generated eid; fire with the deterministic one directly.
  try {
    if(window.fbq) fbq('track','Purchase',{value:value, currency:'USD', content_name:plan, content_category:'website_build'}, {eventID:eid});
  } catch(e){}
  try {
    var body = {event_name:'Purchase', event_id:eid, value:value, currency:'USD', content_name:plan, content_category:'website_build', email:email, event_source_url:location.href};
    if(navigator.sendBeacon){
      navigator.sendBeacon('/api/capi.php', new Blob([JSON.stringify(body)],{type:'application/json'}));
    } else {

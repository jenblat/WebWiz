...
const WW_UPSCALE_WIDTH_THRESHOLD = 800;   // images narrower than this are candidates
...
function ww_upscale_html_images(string $html, float $budget_usd_remaining, ?int $job_id = null): array {
    ...
    foreach ($m[0] as $idx => $fullPath) {
        ...
        if (!$dims) continue; // unknown size: skip (we already downscale to 1600px in img.php so cache exists for proxied URLs)
        if ($dims[0] >= WW_UPSCALE_WIDTH_THRESHOLD) continue;
        // upscale
        $up = ww_replicate_upscale($orig, 2, $job_id);
